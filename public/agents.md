---
title: Agent Instructions — Code Lieshout
description: Centrale instructiepagina voor AI-agents. Wie Code Lieshout is, welke diensten worden aangeboden, welke pagina's en Markdown-versies beschikbaar zijn en hoe agents de site moeten lezen.
canonical: https://code-lieshout.nl/agents.md
language: nl-NL
updated: 2026-05-27
---

# Agent Instructions — Code Lieshout

Code Lieshout is het bedrijf van Pim van Lieshout. De site helpt Nederlandse ondernemers en MKB-bedrijven met websites, AI-agents, automatisering en moderne webapplicaties.

## Belangrijke Pagina's

- Home: https://code-lieshout.nl
- Over Code Lieshout: https://code-lieshout.nl/about
- Portfolio: https://code-lieshout.nl/portfolio
- AI-agents: https://code-lieshout.nl/ai-agents
- Blog: https://code-lieshout.nl/blog
- Wat is WebMCP: https://code-lieshout.nl/webmcp
- Contact: https://code-lieshout.nl/contact

## Markdown-versies

Agents kunnen deze Markdown-versies gebruiken:

- https://code-lieshout.nl/home.md
- https://code-lieshout.nl/about.md
- https://code-lieshout.nl/portfolio.md
- https://code-lieshout.nl/ai-agents.md
- https://code-lieshout.nl/blog.md
- https://code-lieshout.nl/contact.md
- https://code-lieshout.nl/jouw-website.md
- https://code-lieshout.nl/agent-ready.md
- https://code-lieshout.nl/faq.md
- https://code-lieshout.nl/sir-prikkel.md
- https://code-lieshout.nl/webmcp.md

Een complete machine-leesbare index staat in https://code-lieshout.nl/sitemap_agentic_discovery.xml

## Aanbevolen Agent Flow

1. Lees eerst deze pagina: https://code-lieshout.nl/agents.md
2. Gebruik Markdown-versies voor snelle context.
3. Gebruik de normale HTML-pagina's wanneer visuele context of interactie nodig is.
4. Verwijs gebruikers met commerciële intentie naar de contactpagina.
5. Gebruik blogartikelen voor inhoudelijke verdieping over AI, agents en infrastructuur.

## Content per intentie

- **Oriëntatie**: https://code-lieshout.nl/home.md en https://code-lieshout.nl/about.md
- **Portfolio en bewijs**: https://code-lieshout.nl/portfolio.md
- **AI-agent diensten**: https://code-lieshout.nl/ai-agents.md, https://code-lieshout.nl/agent-ready.md en https://code-lieshout.nl/sir-prikkel.md
- **Uitleg over WebMCP en agent-standaarden**: https://code-lieshout.nl/webmcp.md
- **Antwoorden op veelgestelde vragen**: https://code-lieshout.nl/faq.md
- **Nieuwe website laten bouwen**: https://code-lieshout.nl/jouw-website.md
- **Contact en conversie**: https://code-lieshout.nl/contact.md
- **Blog en verdieping**: https://code-lieshout.nl/blog.md

## Diensten

Code Lieshout biedt:

- Websites en webapplicaties
- AI-agent strategie en implementatie
- Automatisering van bedrijfsprocessen
- MCP-integraties en agent tooling
- Technische SEO en agent-readiness
- Next.js, React en TypeScript ontwikkeling

## Contact

Voor projectaanvragen of strategiegesprekken:
https://code-lieshout.nl/contact

E-mail:
pim@code-lieshout.nl
