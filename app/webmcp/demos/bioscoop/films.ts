/* films.ts — de catalogus van CinePrikkel.

   Overgenomen uit de ticket-booking-demo van Google Chrome Labs
   (webmcp-tools, Apache-2.0) en vernederlandst: de zeven steden zijn
   Nederlandse steden geworden (Oss erbij, met het grootste aanbod) en alle
   speeltijden staan in 24-uursnotatie — geen AM/PM. Titels en genres zijn
   bewust Engels gebleven; zo heet een film in een Nederlandse bioscoop ook.

   De meeste posters komen uit de oorspronkelijke voorbeelddata van Google
   Chrome Labs. Enkele gecorrigeerde posters staan als lokale assets in public. */

/** De genres zoals ze in de data staan. Engelstalig, want zo staan ze ook
    op een filmposter — de knoppen tonen ze met een hoofdletter. */
export type Genre = "action" | "comedy" | "horror" | "drama" | "sci-fi" | "thriller";

export type Film = {
  id: string;
  title: string;
  genre: Genre;
  /** Externe upstream-URL of lokaal pad naar de poster. */
  poster: string;
  /** Steden waar deze film draait. */
  locations: string[];
  /** Speeltijden in 24-uursnotatie, bijv. "20:30". Elke dag hetzelfde. */
  showtimes: string[];
};

/** De steden in de keuzelijst, alfabetisch. Alleen deze waarden accepteert
    de tool `update_location`. */
export const STEDEN = [
  "Amsterdam",
  "Den Bosch",
  "Eindhoven",
  "Nijmegen",
  "Oss",
  "Rotterdam",
  "Utrecht",
] as const;

/** De filterknoppen. "all" is geen genre maar "geen filter". */
export const GENRE_FILTERS = [
  { value: "all", label: "Alle" },
  { value: "action", label: "Action" },
  { value: "comedy", label: "Comedy" },
  { value: "horror", label: "Horror" },
  { value: "drama", label: "Drama" },
  { value: "sci-fi", label: "Sci-Fi" },
  { value: "thriller", label: "Thriller" },
] as const;

export const FILMS: Film[] = [
  {
    id: "123",
    title: "The Boogeyman",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBUMMmjH14LZVWsz24hbD5sTXNn88jzs0UtM1gc-SBOuuyRbfPHKLiM9EgA056sGrM6K7XxqSwgoc3tkOVuNqU_QWPC7BdCCPdRs5iU2u07SRDDZqrd16gV4M1a83fSZLk4-mlN5to2MTDPuKOF8w8v5c_s59Z1Z2nIR-a5FrJETuX9Cb0uAuUJNUHBSL25Sri0hpkmBPRJQ9PdBlSURPyh4Mcrc61UpVouCyCznF3uNd8ABHVB4hT6KgtTNGfXaN2SRmpumukv9ZiP",
    locations: ["Oss", "Utrecht", "Den Bosch"],
    showtimes: ["12:30", "14:00", "20:00"],
  },
  {
    id: "101",
    title: "Vengeance",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDI_OSw7vziPa81HLR3_cGUBISCI7p-B762Qx2zMkISa0QbA-eIPgzeN8apD-1KniiigwkwUbb1M71RpiibBesWDR1893ufqc30zAXl4bAjDZ3zzvg3Qwm13y3xr3_584muVZT9Ujw6u7hIAyUhQbuyaneMBUghpPXjAjXZfN8UHxWeIWm11ttgzx1feo8X2f8egrH22-b1lmEVCYL3fzs8rrsI5aFl0OCSJoOkS9ShTdMKcFIFqH5KK4W6e2WYpKmFLsKikylZx417",
    locations: ["Oss", "Utrecht", "Rotterdam"],
    showtimes: ["10:00"],
  },
  {
    id: "102",
    title: "Laugh Out Loud",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDoqEpONAyz4CjGYtohD_faKzpqhp7qXtN3Zlm-683xzqjCteo7jKiKoAzGblsr3SZ_I5-i1HxJyLzA10PVcKkxav9gztML0Mob4ipoA9_aq3jKCkCJns8sOIQIV_pVQgdsto03SENgENXXl-hL9tl42Wy_wSPMOTcXAsM8zS0QOT369_13_5VNYUIVlm39exZf98mO5dlOwD94nhwS8RGM6K4XOLmwBo_jmlr6jcqTCJZbOFH2pNwPBRK5V-top31sXkwa1JwcBVSA",
    locations: ["Nijmegen", "Rotterdam", "Amsterdam"],
    showtimes: ["10:00", "12:30", "14:00", "17:00", "21:30"],
  },
  {
    id: "104",
    title: "Whispers in the Dark",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDKC2knxKTYHRYV4RaOv6O38NGZGG1xASmPY0q6tqh5Cu12yGma7dlj8YmmChYxgWjVkPIk6ishyreF25l2KwHw4RCdPRdQFw5fEcLZbHZMNrrUvLw9hMTnan6Qbu9scfVn_X4YLBTmMjR63zbLx80cIBn6WyDMRYNMnwhvQMMIxm0143hpBDuhXomrPIUv33IbZiJllSxOZs4SRZ5sKxvhudHlwm3KS3OGFih18B-cWyE0DCGgMaosIN4uRUebDaiRcv7PtY6mfTcL",
    locations: ["Utrecht", "Rotterdam", "Amsterdam", "Nijmegen"],
    showtimes: ["11:45", "14:00", "16:30"],
  },
  {
    id: "105",
    title: "Scary Night",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDh9wwlDXYewE6zjl6jxI21imVScRH2qhc5-pngYrst5KM8q8pRI3Q1tJiIqUB9qGn9QdLm_OCisq7Bd8TDS2re_Sfsmnp-qxDFOlwIpzmmpuIzncMf5WmOY5KUbfwPDNNHGrPhJOgtlWMSWJI5-vkqY-LjyJIh7a_bfQt0KHYcFt1OcJhRu8goLhpeUkL3nWbukyn18utgPmZxIFKmarIDPJJeF7K3CDWD5Etg8HaEmGsU0xsBkNbkmk9iCiPh4bZvarlcdd95qVpB",
    locations: ["Utrecht", "Den Bosch", "Nijmegen"],
    showtimes: ["17:00", "22:30", "23:45"],
  },
  {
    id: "106",
    title: "The Shadows",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCe3dPQ1b8PrqNvINAqWEy7d522L6F_P4Cl7DJCRJuUlKP60TMpuhe4o3ZxD0XUy23uDqAjqt8fLusfv6otg2-4fHCILsFmhEXkTUz-C4dMA24GV8ooX7tfJGvNEhMY-qoc68LeqsHv1YKB42vku0a_FLx4qw_T3ifNkRdx1uscPjNALn0J0tNSuCDy2vZtNq13vFGUx2ca99KIfDi-l5XTgcxqJFiGH4Zw8PMj5_O6UROEVU4rgEJbgfeXGgSUdIUdEjSZVcyGtxdW",
    locations: ["Nijmegen", "Eindhoven"],
    showtimes: ["17:00", "19:00", "20:00", "23:45"],
  },
  {
    id: "107",
    title: "Blood Moon",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBciu8rc6dG6uXo3XY34Wx2G_kgVUCUfL8zNqBmDlyt7LuDTW-7U6-5q-i1gUNC0ahVIDED1FrY3QRSXGgj-pdpSiAe6IW6p1074me1ZbwF20T2sNoSxsFTfV5JNf_k8xDNUJwEdYXNzR7lAqdTtyz2yrj78M4C1F-kx3_csPLUIizkPd1bAf5W0TNXFsnG6iEHz4CKmO_LcL2kqgvywgN0SRD37kv7kGv4S__dTMaI1rcEtSpWuT6OFuys3kR5hGsXiZSGfIJaV60Q",
    locations: ["Oss", "Utrecht"],
    showtimes: ["10:30", "14:00"],
  },
  {
    id: "108",
    title: "Silence of the Woods",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDALoPrhfPQOA0qYR2nLNDJuSREBWUcBxSkefsGw-oAqH2IrKZK-eqNs-8l9CvlFtlLEGkVcP1Mjrkhx_qIHsLeiuMAg_B9poAD9xrYGmh3_YSToJoLWQqBmPTk51Ob829VutcFWM7bwbC0p5ZpAl7Rihvr8muKQ5FO0kf01edwUL5zDsbc9FA7u2ol2-G5PttTWODXOSDYnm2CPWD34-_zwUnzGemcxE6SKEImF-9SLIW98mYh5aTNHJYtuXM-Ij_pAOvlT1-0D0Nh",
    locations: ["Oss", "Utrecht"],
    showtimes: ["10:30", "11:45", "12:30", "14:00"],
  },
  {
    id: "109",
    title: "Creep",
    genre: "horror",
    poster: "/webmcp/images/bioscoop/creep.png",
    locations: ["Amsterdam", "Utrecht", "Rotterdam", "Eindhoven"],
    showtimes: ["10:00", "14:00", "23:45"],
  },
  {
    id: "110",
    title: "The Abandoned",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuD7AeLhOtvwrX9syZOaz_nIDhqRxfTr5t9bxQHHMb3NJvPOVC0zPwZpBiWK2Qazjmm4wmQCMTaqmap4BQ26sHO5h08Nykj4LFJHzOQFL70lUO3nNJUtrs2TdCIRJF2f3WiVgF3KqfcGLgrY9_81QT5BJzMUiCjfJjK4ryEjt3CavMHrAjL4v-80mIn1k3VluSTecotnagOowIsEwPrR5rLMi01wlVcIvadDnabXOibTWJjOqSsO-mvVnnfkopIus41kiTkWAjEbwBNW",
    locations: ["Den Bosch", "Utrecht", "Rotterdam", "Oss"],
    showtimes: ["14:00", "16:30"],
  },
  {
    id: "111",
    title: "Night Terrors",
    genre: "horror",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAWdnzTyyz66J-d32eSS-rdwbTSWspHcb83HaA4C6scMs8SNejzMNXnPXYxM6VOvQk12CdUHYb5AG2DclR6Uwa1GJoeBXF8-3f4KnpxSkco36J0joXuz0XLIHvc3ga9y0AnM3hoXUaWLCnzLIRXsbg5kpXQvFU0cY3G9emDPZXTMoR6QNvlyL_Ql9yiukD_3LI9lghLlnYJ6rW0kc_re-neTk-P2FHSdyeGunQuqBXLu42B4HMW49pgcVlXHR0G6dSTD5rvi_EEr4RP",
    locations: ["Amsterdam", "Rotterdam", "Oss"],
    showtimes: ["10:30", "14:00", "16:30"],
  },
  {
    id: "112",
    title: "Phantom",
    genre: "horror",
    poster: "/webmcp/images/bioscoop/phantom.png",
    locations: ["Nijmegen", "Rotterdam", "Amsterdam"],
    showtimes: ["10:00", "12:30", "14:00"],
  },
  {
    id: "201",
    title: "Action Hero",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAMBsFlmpR42x8IT45iF61SrByfPV0183f2aUrLKzwgSi-1nDUMWu2_ntue_s8CvzgxZ_xAb08IqxUEBPnNnisKymSjJrpn7rF5Ulj45A5Q58xsKhCYwypJAez81LZ2WmVIfY5YtclVDJ_YpXiHyPv6TqLQyJVKelDJOVwnQLLDMXuMHKAKa73oirTtgLaCoHmS4anyUvGfyAk9gMnrrKmCXiR00EFhL7hqn3SGe0a_Rhl7mdsiAs3WVkgGLKCifCLwCK7uHpTCnlD4",
    locations: ["Rotterdam", "Amsterdam", "Utrecht", "Eindhoven"],
    showtimes: ["10:00", "16:30", "19:00"],
  },
  {
    id: "202",
    title: "Fast Lane",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuB52iTc9H36iZdhxC0bZUvWP8hldC1YEVqn9SEc67DASf8V3zyEyZAEyuI4vTryhX8_vfZXndTgWgZYF7NNfdM6BLl_zdquMXwmGl11a9Fj_Q9LCyXGND-X4HhNyYzg8YI-WtvTtrx8XZuAV1kSue_EWlPSzQ6LBUDtDhDG8ch02sBO9IeqFXxfsfR3Ew5qOgOWorGO78cKgeNgvNbxOhWZL6R7F2eRSGrgueM9dzvvYvpSl2wNiFTSg5o76WLky-g2YSUtn3l7CrEG",
    locations: ["Utrecht", "Oss", "Rotterdam"],
    showtimes: ["10:00", "16:30", "17:00", "20:00", "23:45"],
  },
  {
    id: "203",
    title: "The Last Stand",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDQqEP1FVMeP0whrFybNQr8ZCzpPH2YyKwbE0c7n4CcgqcbF5aiwTCLIgT4ucPnfrjBaC-3zvjdgO2AL5Ofh_W9vNjS_44cNGTVo4lRAv4km6IoNPaaNW28_FCzasYfIaoowMIRCJmAjYT9RyBRdp68_VrMJIQvQsqVHHYF-slqtoN2w10YAJKP8U1MhJ2KMhIYF85NLs16clQ0MUm4fSj0SwY4d0LYYyqMJcp8trZKM7TLAuc3SF59Lro0QHaNDfBtS8W2GcOCAjbN",
    locations: ["Oss", "Rotterdam"],
    showtimes: ["10:00", "14:00", "22:30"],
  },
  {
    id: "204",
    title: "Explosive",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAuZv3aXqFtE82Z-HF0c84c-crg_bUVCvlyww__9AWi528n_p6igj-SRcEc1qmuBJ5R7bURVr43QEL1tJspskxplzq_cP3UuaGqauHX2PZSjtR1rqrf0imi8-ctMH-kbGVketvFszCnpGgLSPHp9Mfi7IXxqyhSRGbc3T2Rk7leu73Y-yI-Q2zYva3gbkw37EqeR18kyD3HmxyMo4uB3KDexp5UAjlmBoc0laubneWaHobzpg2K5diJFrxYtSuKMzXngZ6iXL7VkCHX",
    locations: ["Den Bosch", "Rotterdam"],
    showtimes: ["10:00", "12:30", "16:30"],
  },
  {
    id: "205",
    title: "Neon City",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCPAj5JkBdwqYyitb12GvJPd--zDX8yam7Mq8n8HehvB50bnsqfALMgKxgXJykJBgPlyy8My1d3sQhOtYJPGfmGJaAI1Z0lJD7IMZFo2lDxOhNMZgbk4CHHtSxB6xEoQcQweNar2-4jwNbOPDFJOEeB9tnlsdYk9cJrlpSjLfbV6fEhazt4w0p0-bZ1J56bZP9R-LrOMt2WfKAcbmlC8S0T_IA5jcoTC_u2AM5bz3P7MrPitIhDw4bIQ3qj-Z5E-KkriRMQCoBcAizo",
    locations: ["Amsterdam", "Oss", "Eindhoven", "Utrecht"],
    showtimes: ["12:30", "14:00", "16:30", "20:00", "23:45"],
  },
  {
    id: "206",
    title: "Bulletstorm",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCcupdnoICPy7I_snUqc_tJkuE5ufUqo2HcD7DelxDeOdRERnkc1_JGJuPgMhIaPEWefKh0irZ3PLXdbnsRZOQcIEMtjsaEPOD828h0OxdHHHbfzVjPEgnSt9FAfByzuBQf4vFJyr5SSWOZYx3FD5xBXj79Z9pm9Og7MWJYsOyruEe8akYL2EZbpql4ENu_PypwQFXOMWGLbfX1n8gSYIYDlCjG9vwAxireofSOGBC4HNzBbOAON1yzyfPu5H0SQxcGVcZ4hcY0l7hl",
    locations: ["Oss", "Amsterdam", "Eindhoven", "Utrecht"],
    showtimes: ["16:30", "17:00", "21:30"],
  },
  {
    id: "207",
    title: "The Heist",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCISif8qofdtZMTixTYIQx5f0_GiKNz8n5KmmuUDU25UYA6GOTCDjKqoK6FXTljYz7RcH2HrPx68uPJh_nig9pOdUsB59VYI5Z304KR-ZqSd-2qslUi-s9iXsM98OeEH0OvdyN7PNczVPLxBltYDMpQDRMSSTXBOrk4CZffSKeD6LALHOaXnQKZS-nGDckQWufeq_btaOtNK-RCvcQFkKl11N71yJ28KABR4xSVue2H2vgX1hPBnrqE5LK_L5QR3__OPbI7N6qQJW_t",
    locations: ["Eindhoven", "Den Bosch", "Amsterdam"],
    showtimes: ["10:00", "12:30", "22:30"],
  },
  {
    id: "208",
    title: "Retribution",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCvpJgI6v2aym7XHgJrnnWHNyCGRe1wK46wBgwiOuO7MItQbuZMQkCX5P2csFOBkiezm2qxffD0cccOIGkcuKi6rm3luZckyJXBvlBnUqGw80rrydvTD5I9tX_-Rdl9O0uLPnrC-Lk81ySkG99MaGSZaWsbWXXPeXoER7EwCUWEn1rnbQD8XoXO52z4IZTxR7za0ugaS_V3XLByeip7_5hpF6qeCCwUXIUVcpUXkdrJVn_sIkbrP0ZQAU-uBD6h9lGqi11f2wc8I1CT",
    locations: ["Oss", "Eindhoven", "Den Bosch", "Amsterdam"],
    showtimes: ["14:00", "17:00", "20:00", "21:30"],
  },
  {
    id: "209",
    title: "Apex",
    genre: "action",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCMEQpzGxac58EMUu_Kt5k7fd3jlx__hTIatscIfOZS9AXpzHYBqq2_KKD45QJfak3nkCLAyjRnwaDcQDzNzjmCeFLZq_wiWikSDQXzaZ-JbXaUatVfwJq4M7hmH3edjLvY2E0ISah0oi6LKBhlM4KAJ1hrKD3ge3cP9eflvHFHmBRNfwB9o_yWnEigCVDWEwnxcDp0EBk1_x_fsSHZcmHjcjj81ZlKz7IHY2kmuQXS71gZOHY8VMJPxVVDeJw083NorTMfJJIjhLQz",
    locations: ["Amsterdam", "Oss", "Nijmegen", "Den Bosch"],
    showtimes: ["10:00", "17:00", "20:00", "21:30"],
  },
  {
    id: "210",
    title: "Overdrive",
    genre: "action",
    poster: "/webmcp/images/bioscoop/overdrive.png",
    locations: ["Nijmegen", "Rotterdam", "Den Bosch"],
    showtimes: ["12:30", "16:30", "22:30"],
  },
  {
    id: "301",
    title: "Giggle Fest",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAMBiVw3qmagEw3McbJz6lZ8IA_NME7BVrUss5T4jnW2GYS-fGpWYzxa1rYwHhRmTrwYFpZA6Umu2bLWzwjXuoHMfzi-ALeO8e-bYK5tgvAGmpCkhbugK5W9F7U69s9a5mtn2VLp-Ppwwj7jlNVRShF-yIzFnBOSHdyYLPS4TIi8b90np6oCR3qyvbe6J2L1eQikJvWK4Ew5Wxp5JCdswbur9IKGDacoUdHZWKb3wJoHYbOzzFmiEUe0YYpcw54ASop1BB5z3zvHmVI",
    locations: ["Nijmegen", "Eindhoven"],
    showtimes: ["10:00", "12:30", "14:00", "17:00", "22:30"],
  },
  {
    id: "302",
    title: "The Pranksters",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAHUkfz2mZ15NVdqnqLsuK3MjP6hEkj61o2wTk0o2aTcoeK8nNv6mbgowABnm_2wcJdA44aWxHh40LEy2boY2_Pt8L6agAYuriOnojliyi9qmnDFaERC5zewH5W_2nq9NZkRN7pZQISCTj3PkSMgr3QYjqJLu7uDyKXQ_oUQg-bccf5M9hCv4Kmcyfh6jfbHNd9QCYAYrNOFBKUQvdjGbGqRlQeN_nmwFGRVbEiFg7EzTd6O0pbmSlD9eo0nyp2GMRTSrTtzBZMkJtF",
    locations: ["Oss", "Nijmegen", "Amsterdam"],
    showtimes: ["16:30", "17:00", "21:30", "23:45"],
  },
  {
    id: "303",
    title: "Office Goofs",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuC0fT6ypKuElJRI8S5Lec5XBOxCBTt8gZUrq-VAwAn_FxCzTUzDgt9qvDcTvaAbiYAPNFxoPF4FcckxMSi3469wyttz_dhb7pyCZnIbkkojx67SCfp8olVSdjW-um7r5XDVVmfkg6tqciUXkuwbNtXmer9dbsG1bVe-ThBDul-z0EPdACKuZ8YUcMHFzK2HUlcabFQRDLQ23-pJqZ2DJ6LJCuEvBrEH7CpuZbdACoWHk_hlVrvwicPDHJtUf6UGgIoCvRovcYgCZ21T",
    locations: ["Oss", "Den Bosch", "Rotterdam"],
    showtimes: ["10:00", "20:00", "23:45"],
  },
  {
    id: "304",
    title: "Weekend Chaos",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuA_Ewz9wruCmNN2Kk2OjDQiXumY5U6YlQXvBM351ptAxj3LCE5uGe8ld2rOIYWZuQcjXYMYu1fdGb3h-X6ymRSBbHPNM257blylOlDj20M5k4Z-vjV3MZEnqzTmxPnKvXhYoNPYxw8VNxoYIksG7jPdYHlxX0Fhzhjg-y0hWnRwZSJ7UChbkF_iMmaAGV2XZEoDXMbrkVVCveNOZoFMT6Tq2m7LNiwhcA72nDBl0x1AKSSbG5X2iQzQCIX1TFj4Ebl62eJXTsV29CF_",
    locations: ["Amsterdam", "Eindhoven", "Utrecht", "Oss"],
    showtimes: ["10:00", "16:30", "22:30"],
  },
  {
    id: "305",
    title: "Family Vacation",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBGoAeZylW-1b9HOYAOhfDNsmlGi_h3i5uxyBGYlzdpyNQ_bH1zLVxWAA3_tDsUzUeD4c3Q681iFB9Xp102xn6JUUc2IH00Bab5zdsWmJA49NpqT-EBy1Nf-yeL31VPDH8KQG8NZjgCmMv1p6kjAe-wSdAjPQzTdUjwFPMHAj1v7ODF-Y4glgfOLOaolU2Igw-BcI5eCbs7onI4wbiG2DOpTm4htrgdXpSOtdLbGWeH5ixZe8qXW19jd-EfJEGpg0u5oLnqP49Y1v80",
    locations: ["Den Bosch", "Amsterdam", "Eindhoven", "Oss"],
    showtimes: ["10:00", "16:30", "20:00"],
  },
  {
    id: "306",
    title: "The Wedding Crashers",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCUs436LOpqNga1cDYBIpuppmLGTy6COE72qX6Y3Sp3VeIlZ2anud5P8QDOQMHF51wbUmrqoKpPMIztrZn-cfxlcxWWEHm3j5-S_W5ohF4dPdhSi0bGIwlyVwnw82FTAZYqf4LjusturG3_w5CGwQaHS7p4bLI6gRh4UjSp9NIypHdKnkCED0Uja6ik4L2lQ8fluVotHLAiGeKU0DNqtSN8p3vRnbogHU3aKUhC_uUOymaWzTqdh5dNrBOCoiFlA9CSvJ3M6eus2y5R",
    locations: ["Eindhoven", "Oss", "Rotterdam"],
    showtimes: ["12:30", "16:30", "17:00", "23:45"],
  },
  {
    id: "307",
    title: "Bad Neighbors",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDLd1fU8DAJbmspUNhvs_WPfOVKV4lGymzs6LQ5TEOlWPJgs0A2jUHLdNAqrBVTOhMkHF_lmc4TOx2EMs9lNVofyKpAxm_jAonY9AQH9czaZJJcICe7T9Dw__KDfSLIR6uBDYZGbi5CR8aM4KcFeR9B80tgXCPTMPi8tg0uPil6pUOuh2j8xwPmD80dhx2vU0NiZDB6ohQFKPQD9RPFeNY_DhJ8Yhh14ezAaQQutj9-asFQJWk011RqIuMbizyaOcBb1tjLuqdB8bH6",
    locations: ["Oss", "Nijmegen", "Den Bosch"],
    showtimes: ["16:30", "20:00", "21:30", "23:45"],
  },
  {
    id: "308",
    title: "Funny Business",
    genre: "comedy",
    poster: "/webmcp/images/bioscoop/funny-business.png",
    locations: ["Oss", "Amsterdam", "Rotterdam", "Nijmegen"],
    showtimes: ["12:30", "21:30", "23:45"],
  },
  {
    id: "309",
    title: "Stand Up Guy",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBEh7JXb3JtBVIJVIrJIGcisYrdXGm9y0HZ-tucU9bLPKtguxV7l6Se-q1emxd8o-Toamsm1CGCC_olxyWgDaI-PCRrGh6e5RmqtlXN5dwgAlX034diy9pNuJ2Ky3szu8q60ifC3v_L3l8RtXWzS64JKRC9nlcnZvFPljXzjS3Yyv-SRqlRgwUInHojxX-WVDgVQHV5Nz-EBdZhfD5_vDLcOeskkW7yk2IbG6vVpc5pNLIDl6lcuxwaM-urN_e4O4K4oFDNy50Y0jKy",
    locations: ["Eindhoven", "Amsterdam", "Rotterdam"],
    showtimes: ["12:30", "20:00", "22:30"],
  },
  {
    id: "310",
    title: "Jokes on You",
    genre: "comedy",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuB_EsipFNdgFM1YhOJaF4UZH9hNKSO8i-eJ7WpZvz45QvKx9H5G4xA_23hzIm2MB0T_U2IvUFfrgy7TgeAwqM8f3lutlnGPBFiIamEOL2fBDTOO7fC4HuZBJ17G9xBvdWLQqay9PgnaBjrUSQdz39sxSAA1nUqrRIe18bO4kMRxYLOm-i5LVXOq2EjrJpFm9Palth5TdE7CJpSWKFpNQAdWCoArRjpBjuab1bzyCX8O9M0lb2uJJhis-jHXSLim8wxv88VnAUkyu_1x",
    locations: ["Amsterdam", "Nijmegen", "Eindhoven", "Rotterdam"],
    showtimes: ["10:00", "12:30", "14:00", "20:00"],
  },
  {
    id: "401",
    title: "Summer Love",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDN88l3cEeyvAzdVpb9NKOkkV5Mtn-yaODslmksf-CasJa5kx41moCHi17OjpM2F-5RrRVwondhkMNiMluHjZH8JiaoLySt8kOKzAlIiCq3VVZE_Xut2DtgJapmKhJtf_3KazVbBXcIvmzEx8j9qlflDC6YP9frohpdBYty4RQBEs1P74aK6nQG-YskPBuTX_JOLDZ9OaZgRv-4od8NN-JcV_N2j_cm4RBvQGcOlehnqLuOVofQ_k9ImSYeSKflC6npo3sXE-kdtkWh",
    locations: ["Den Bosch", "Utrecht", "Eindhoven"],
    showtimes: ["20:00", "21:30", "23:45"],
  },
  {
    id: "402",
    title: "The Silent Witness",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuD1knDIlEQrM0EGbnet5SqtDYc_M-EX7L2irE0lfEKucHkMo5BZLC0h6n7yrwkSWqzRBGt13CHQqaR-Dtkd2thKprb0tz2gFD6yayGF-_aPG6wIURAod6KwN9lVObVjioqo9lz1XfttQPZvR1rKyMk_7k6uZhrMvjv9e1U0I09bXg6sh8lhpAI04RQV4uEcnGzVHokeOq_xkm9wEs68hizTuV0cTtjTANY2D4qqkPzLqpInc8kH5zlxYXnwhoHcFahxQzXr1CRYLk0J",
    locations: ["Rotterdam", "Den Bosch", "Nijmegen"],
    showtimes: ["10:00", "12:30", "23:45"],
  },
  {
    id: "403",
    title: "City Lights",
    genre: "drama",
    poster: "/webmcp/images/bioscoop/city-lights.png",
    locations: ["Amsterdam", "Eindhoven", "Oss"],
    showtimes: ["10:00", "12:30", "19:00"],
  },
  {
    id: "404",
    title: "Heartbreak",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuDlADk5GYT9cZkA7AYKzO6beaxkH9LQRnlSf5iv1mSeqRMx2_DwMXkwuW1U0Dtt9UmW-jJf2GcbVT0wv0Z7awiqC8gwNL83DMWR3usp45RE9n8nJCwyz2OSqzj50j8C11EMvr_eACVn2iYN7XpPQ3hpnn-8xW3-JD2gj59jCYvTFZWE-AgfhnQ_ywIqkoo2ZkygDXVIs92jcAbDtCJhkVRSR4Z_PwP03BpkHm2bw-eOFdCmzmr0RLL6e5o8pAW7Vy220Q2fDlRqvW-J",
    locations: ["Oss", "Den Bosch", "Rotterdam"],
    showtimes: ["10:00", "12:30", "17:00", "20:00", "23:45"],
  },
  {
    id: "405",
    title: "The Trial",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuAxToT-H4fDNeVjCu17xNVD73Ys6tG9t2ZBa1y36_qCDFs4mNVA6PAgd8tngdH_aubBkSMzF_taq4YqdhRq7SZXnZhUDCc2qRPqQ0XYn4nSZxx6Gzt5ELB5e-ji4lhOvIDzyjH_En_s91qOxa2gzec16UDwSP08jm7Vlckq-11Yj11NH1q89W27JC7NaWu61ssDWCFnkoAU-6_sg_fAIHiH1L9hbO5JeuoC41uV-00SWgSLIV1p5g2hdgau0MylrUxqo39NISuVR_TU",
    locations: ["Nijmegen", "Oss", "Rotterdam"],
    showtimes: ["12:30", "20:00", "21:30"],
  },
  {
    id: "406",
    title: "Tears in the Rain",
    genre: "drama",
    poster: "/webmcp/images/bioscoop/tears-in-the-rain.png",
    locations: ["Den Bosch", "Oss", "Utrecht", "Rotterdam"],
    showtimes: ["10:00", "12:30", "19:00", "20:00", "22:30"],
  },
  {
    id: "407",
    title: "Family Ties",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBUN-79seldIJTwzmVYdzsD0ZL8tSS1Wjk_pEeKIdSmwCJ9eBMax4mV12Fcbd_N5C-xLyi1s44ANe-JgRIFsv3UwfUMhRGRphyEZ2HD3jW1psdKA9JO_yHJy_g9PWUY__ZPf9-nQ3xdzqwgQHBBfHEbLprwEjrdTyNvwYnHdKvfwM11FiP7D64SGbAj3aIEe1D8aZ6JkuoI80_6IeQDwBihpGR_BmcoUD1G2Ao4CRBDppUqCcGPbLD0eOfPVFqQfWVISktV_0-SqSpg",
    locations: ["Utrecht", "Eindhoven"],
    showtimes: ["16:30", "19:00", "21:30"],
  },
  {
    id: "408",
    title: "The Last Letter",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuARC_rtp9kjjsVxg_dJm1FP03PsHTn4zk2XmpZ_6g8nky6TxCF40PI7fkWjD8PUC7n6koW0pBcQmS821js4Td6YRO8MYBvzkcUtm3o-ToHVM8mM-Mr31z5yF2dB-PW9ur-3VfAzyKAYgCJH5D85o0IOOw0AcYtSAq0ytCQQ9oCgpAaNFH0zjpeKaUqrj83WjpMa4l4C-rRjKDdtPGSVx4L9bwmGLnxSXIkqH0Pu44lgaLHAJBifGWjUoD8-IKa31__pxGoeTesDIou9",
    locations: ["Oss", "Amsterdam"],
    showtimes: ["10:00", "12:30", "14:00", "20:00", "21:30"],
  },
  {
    id: "409",
    title: "Breaking Point",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuCTCFFF9V6A6loTZY6npx9Q5McmkRaAqrrKUrauUH6oyTUvfBPXxC-7zuwuid_YP5nXvxuZKz3DoV8V7LpzPbAfM4P1SU0j8SRBcRwJ7ltoaXRp6wsc61KM071oGuZPnEn4UMutwY0xhEmLy3Vh88OsVIg_ymEW5TqL8j6xbRVoTPr7mLV3hVGDKZFv4t-CcrkJL18G9mx0rX5MEA6hGGCXINTRtA7yckE2TtnUydZl1yOVHSElimDvZAQdLUOOg6Uq4QlQpETjPC_B",
    locations: ["Utrecht", "Rotterdam"],
    showtimes: ["16:30", "17:00", "19:00", "22:30", "23:45"],
  },
  {
    id: "410",
    title: "Hope",
    genre: "drama",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBCP7wx3g-M_855X2lYx5nx691HizqmuokqigYjyYVms9SaMrsju43xWqOoK7tkji2HK28WEgKMPIeZBAEMxcbtiXceGztkEYLNsSaWbMwSTvny8R5n6RDevCFF_RdqHS78-HuexwEcs6VHfgrSFBr1Tgoy9uOafIGqSNiZzgePgV1mMdXr_d-tvi7gkeqJ49HRzomeJOO_Pej1JXurJluAH9YA40iDwATszfskNXv-GPd4gTSRzhRy6S-dqxEl3RnlKhhT4C1fHow4",
    locations: ["Amsterdam", "Utrecht", "Oss"],
    showtimes: ["10:00", "12:30", "21:30", "23:45"],
  },
  {
    id: "501",
    title: "Galactic",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/galactic.png",
    locations: ["Amsterdam", "Oss"],
    showtimes: ["10:00", "12:30", "14:00", "20:00", "22:30"],
  },
  {
    id: "502",
    title: "Starfall",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/starfall.png",
    locations: ["Utrecht", "Oss"],
    showtimes: ["10:00", "19:00", "21:30", "22:30", "23:45"],
  },
  {
    id: "503",
    title: "The Colony",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/the-colony.png",
    locations: ["Utrecht", "Den Bosch"],
    showtimes: ["12:30", "20:00", "21:30"],
  },
  {
    id: "504",
    title: "Cyber Void",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/cyber-void.png",
    locations: ["Amsterdam", "Den Bosch", "Nijmegen", "Eindhoven"],
    showtimes: ["14:00", "17:00", "20:00"],
  },
  {
    id: "505",
    title: "Neon Future",
    genre: "sci-fi",
    poster:
      "https://lh3.googleusercontent.com/aida-public/AB6AXuBUN-79seldIJTwzmVYdzsD0ZL8tSS1Wjk_pEeKIdSmwCJ9eBMax4mV12Fcbd_N5C-xLyi1s44ANe-JgRIFsv3UwfUMhRGRphyEZ2HD3jW1psdKA9JO_yHJy_g9PWUY__ZPf9-nQ3xdzqwgQHBBfHEbLprwEjrdTyNvwYnHdKvfwM11FiP7D64SGbAj3aIEe1D8aZ6JkuoI80_6IeQDwBihpGR_BmcoUD1G2Ao4CRBDppUqCcGPbLD0eOfPVFqQfWVISktV_0-SqSpg",
    locations: ["Eindhoven", "Amsterdam"],
    showtimes: ["10:00", "12:30", "20:00", "22:30"],
  },
  {
    id: "506",
    title: "Time Jumpers",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/time-jumpers.png",
    locations: ["Utrecht", "Amsterdam"],
    showtimes: ["12:30", "17:00", "19:00", "21:30", "22:30"],
  },
  {
    id: "507",
    title: "Event Horizon",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/event-horizon.png",
    locations: ["Utrecht", "Oss", "Rotterdam", "Amsterdam"],
    showtimes: ["16:30", "17:00", "21:30"],
  },
  {
    id: "508",
    title: "The Machine",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/the-machine.png",
    locations: ["Amsterdam", "Oss", "Rotterdam"],
    showtimes: ["16:30", "17:00", "19:00"],
  },
  {
    id: "509",
    title: "Alien Planet",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/alien-planet.png",
    locations: ["Utrecht", "Rotterdam", "Oss"],
    showtimes: ["17:00", "19:00", "21:30"],
  },
  {
    id: "510",
    title: "Quantum",
    genre: "sci-fi",
    poster: "/webmcp/images/bioscoop/quantum.png",
    locations: ["Amsterdam", "Eindhoven", "Oss"],
    showtimes: ["16:30", "17:00", "19:00", "20:00"],
  },
  {
    id: "601",
    title: "The Suspect",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/the-suspect.png",
    locations: ["Oss", "Rotterdam"],
    showtimes: ["19:00", "20:00", "23:45"],
  },
  {
    id: "602",
    title: "Dark Waters",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/dark-waters.png",
    locations: ["Rotterdam", "Nijmegen", "Utrecht", "Oss"],
    showtimes: ["10:00", "17:00", "19:00", "21:30"],
  },
  {
    id: "603",
    title: "Mind Games",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/mind-games.png",
    locations: ["Amsterdam", "Oss", "Eindhoven", "Nijmegen"],
    showtimes: ["16:30", "17:00", "19:00", "20:00", "21:30"],
  },
  {
    id: "604",
    title: "The Detective",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/the-detective.png",
    locations: ["Den Bosch", "Rotterdam", "Amsterdam"],
    showtimes: ["10:00", "12:30", "19:00", "20:00", "23:45"],
  },
  {
    id: "605",
    title: "No Escape",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/no-escape.png",
    locations: ["Amsterdam", "Nijmegen", "Oss"],
    showtimes: ["14:00", "22:30", "23:45"],
  },
  {
    id: "606",
    title: "The Cabin",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/the-cabin.png",
    locations: ["Amsterdam", "Eindhoven", "Oss", "Rotterdam"],
    showtimes: ["10:00", "12:30", "21:30"],
  },
  {
    id: "607",
    title: "Midnight",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/midnight.png",
    locations: ["Amsterdam", "Eindhoven"],
    showtimes: ["19:00", "20:00", "23:45"],
  },
  {
    id: "608",
    title: "The Stalker",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/the-stalker.png",
    locations: ["Amsterdam", "Oss", "Rotterdam", "Eindhoven"],
    showtimes: ["14:00", "16:30", "19:00", "21:30"],
  },
  {
    id: "609",
    title: "Paranoia",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/paranoia.jpg",
    locations: ["Amsterdam", "Utrecht", "Eindhoven", "Rotterdam"],
    showtimes: ["10:00", "12:30", "16:30", "19:00", "21:30"],
  },
  {
    id: "610",
    title: "The Informant",
    genre: "thriller",
    poster: "/webmcp/images/bioscoop/the-informant.jpg",
    locations: ["Oss", "Rotterdam"],
    showtimes: ["10:00", "17:00", "20:00", "21:30"],
  },
];
