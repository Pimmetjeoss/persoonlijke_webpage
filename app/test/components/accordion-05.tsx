...
export function Accordion05() {
  return (
    <div className="w-full max-w-3xl mx-auto">
      <Accordion defaultValue={['5']} className="w-full">
        {items.map((item) => (
          <AccordionItem value={item.id} key={item.id} className="last:border-b">
            <AccordionHeader className="text-left pl-6 md:pl-14 overflow-hidden duration-200 cursor-pointer">
            ...
            <AccordionPanel className="pb-6 pl-6 md:px-20">
              {item.content}
            </AccordionPanel>
          </AccordionItem>
        ))}
