...
export function ScanForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    setSubmitting(true)
    try {
      const res = await fetch("/agent-scan/api/scan", {
        method: "POST",
      ...
      router.push(`/agent-scan/${encodeURIComponent(json.data.domain)}`)
    } catch (err) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-col gap-3">
      ...
      <p className="text-xs text-gray-600">
        Gratis. Geen account nodig. De scan loopt via Is Agentic en duurt
        meestal minder dan een minuut.
      </p>
    </form>
  )
}
